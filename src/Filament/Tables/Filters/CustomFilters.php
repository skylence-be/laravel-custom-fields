...
class CustomFilters extends Component
{
    ...
    protected function createFilter(Field $field): BaseFilter
    {
        $filter = match ($field->type) {
            FieldType::CHECKBOX => Filter::make($field->code)
                ->query(fn (Builder $query): Builder => $query->where($field->code, true)),

            FieldType::TOGGLE => Filter::make($field->code)
                ->toggle()
                ->query(fn (Builder $query): Builder => $query->where($field->code, true)),

            FieldType::RADIO => SelectFilter::make($field->code)
            ...
            FieldType::SELECT => $field->is_multiselect
            ...
            FieldType::CHECKBOX_LIST => SelectFilter::make($field->code)
                ->options(function () use ($field) {
                    return collect($field->getTranslatedOptions())
                        ->mapWithKeys(fn ($option) => [$option => $option])
                        ->toArray();
    ...
    protected function createConstraint(Field $field): Constraint
    {
        $constraint = match ($field->type) {
            FieldType::TEXT => match ($field->input_type) {
                TextInputType::INTEGER => NumberConstraint::make($field->code)->integer(),
                TextInputType::NUMERIC => NumberConstraint::make($field->code),
                default => TextConstraint::make($field->code),
            },

            FieldType::DATETIME => DateConstraint::make($field->code),

            FieldType::CHECKBOX, FieldType::TOGGLE => BooleanConstraint::make($field->code),
            ...
            FieldType::CHECKBOX_LIST => SelectConstraint::make($field->code)
                ->options(function () use ($field) {
                    return collect($field->getTranslatedOptions())
                        ->mapWithKeys(fn ($option) => [$option => $option])
                        ->toArray();
        ...
        return $constraint->label($field->getTranslatedName());
    }
}
